...
RUN addgroup tenacity \
    && adduser -G tenacity -D -h /tenacity tenacity

WORKDIR /tenacity

COPY --from=go-builder /code/build/tenacityd /usr/local/bin/tenacityd
...
CMD ["/usr/local/bin/tenacityd", "version"]
