...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=tenacity \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=tenacityd \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/tenacityd ./cmd/tenacityd
endif

build-linux:
	mkdir -p $(BUILDDIR)
	docker build --no-cache --tag onchainengineer/tenacity ./
	docker create --name temp onchainengineer/tenacity:latest
	docker cp temp:/usr/local/bin/tenacityd $(BUILDDIR)/
	docker rm temp

build-linux-with-shared-library:
	mkdir -p $(BUILDDIR)
	docker build --tag onchainengineer/tenacity-shared ./ -f ./shared.Dockerfile
	docker create --name temp onchainengineer/tenacity-shared:latest
	docker cp temp:/usr/local/bin/tenacityd $(BUILDDIR)/
	docker cp temp:/lib/libwasmvm.so $(BUILDDIR)/
	docker rm temp

install: go.sum 
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/tenacityd
...
draw-deps:
	...
	@goviz -i ./cmd/tenacityd -d 2 | dot -Tpng -o dependency-graph.png
